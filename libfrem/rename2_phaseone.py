...
idx = 1
imgs_rec = sorted([i for i in os.listdir(rect_p) if i.endswith('.IIQ')])
for i in imgs_rec:
    org = os.path.join(rect_p,i)
    new = os.path.join(rect_p,create_name("m",numero_codice,None,"visp20_",idx,"a","00","IIQ"))
    os.rename(org,new)
    idx += 2

# il primo è il secondo della serie rovesciato
count = 2 
imgs_ver = reversed(sorted([i for i in os.listdir(ver_p) if i.endswith('.IIQ')]))
for j in imgs_ver:
    org = os.path.join(ver_p,j)
    new = os.path.join(ver_p,create_name("m",numero_codice,None,"visp20_",count,"a","00","IIQ"))
    os.rename(org,new)
    count += 2
count-=1 # ritorno all'ultimo

img_det = sorted([i for i in os.listdir(det_p) if i.endswith('.IIQ')])
...
for fname in img_det:
    ...
    if dorso:
        new = os.path.join(det_p,create_name("m",numero_codice,None,"visp20_","0","a","00","IIQ"))
        dorso = False
    else: 
        new = os.path.join(det_p,create_name("m",numero_codice,None,"visp20_",count,"a","00","IIQ"))
        count +=1
    os.rename(org,new)
    

imgs_tg = sorted([i for i in os.listdir(tg_p) if i.endswith('.IIQ')])
for k in imgs_tg:
    org = os.path.join(tg_p,k)
    new = os.path.join(tg_p,create_name("m",numero_codice,None,"visp20_",count,"a","00","IIQ"))
    os.rename(org,new)
    count+=1

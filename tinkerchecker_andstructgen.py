...
class UpdateLabel():
    def __init__(self):
        ...
        def genera_struttura():
            fogli = ck(self.fogli)
            ante_elements = ['piatto anteriore','risguardia anteriore',]
            post_elements = ['risguardia posteriore', 'piatto posteriore',]
            header = ['parte','componenti','descrizione','tipo','elemento','sottoelemento','disambiguatore','note','materiale','numerazione_A','numerazione_B','numerazione_C','bifoglio','lato','fascicolo','attaccatura','palinsesti','filigrana','titolo','livello_titolo','non_prima','non_dopo','dettagli_materiale','fori_di_preparazione','peli_residui','aree_di_scalfo_residue','illustrazione','foratura','rigatura','postille','notabilia','note_conservative','larghezza_mm','altezza_mm','online_links','colore','spessore']
            ...
            with open(path, 'w', newline='',) as csvfile:
                ...
                for i in ante_elements:
                    ...
                    d['tipo'] = "d"
                    d['elemento'] = int(count/2)
                    d['sottoelemento'] = next(sides)
                    cdxst.writerow(list(d.values()))
                    count += 1
                count = 2
